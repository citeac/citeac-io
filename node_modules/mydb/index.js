...
Server.prototype.onConnection = function(socket){
  ...
  if (this.pending[id]) {
    ...
    for (var i = this.pending[id].length - 1; i >= 0; i--) {
      var sub = this.pending[id][i];
      debug('flushing sub "%s" for "%s"', sub.id, id);
      client.add(sub);
    }

    delete this.pending[id];
  } else {
    debug('no pending subscriptions for client %s', id);
  }

  var subs = Object.keys(client.subscriptions);
  debug('current subscriptions for client %s', id, subs)
...
Server.prototype.onclose = function(client){
  ...
  if (this.pending[id]) {
    debug('destroying pending subscriptions for %s', id);

    for (var i = this.pending[id].length - 1; i >= 0; i--) {
      var sus = this.pending[id][i];
      debug('flushing sub "%s" for "%s"', sus.id, id)
      sus.destroy();
    }
...
Server.prototype.subscribe = function(data, fn){
  ...
  debug('initialized subscription "%s" for socket id "%s"', sub.id, sid);
  var start = Date.now();

  if (this.ids[sid]) {
    debug('not buffering sub "%s" for "%s"', sub.id, sid);
...
Server.prototype.buffer = function(sid, sub){
  var self = this;
  debug('adding subscription "%s" to pending cache for "%s"', sub.id, sid);
  ...
  function ondestroy(){
    debug('removing subscription %s from pending cache', sub.id);
